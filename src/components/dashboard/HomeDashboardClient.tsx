...
function Sparkline({
  ...
  if (!values || values.length === 0) {
    return <div className="h-7 w-[120px] rounded-lg bg-white/[0.03] border border-white/5" />;
  }
...
export function HomeDashboardClient() {
  ...
  useEffect(() => {
    let isMounted = true;
    const sync = () => {
      if (isMounted) setSnapshot(readPlannerSnapshot());
    };
    sync();
    void syncPlannerSnapshotFromServer().then((serverSnapshot) => {
      if (isMounted) setSnapshot(serverSnapshot);
    });
