...
export function calculateWilksScore(totalKg: number, bodyweightKg: number, isFemale: boolean = false): number {
...
export function calculateDOTSScore(totalKg: number, bodyweightKg: number, isFemale: boolean = false): number {
  if (bodyweightKg <= 0) return 0;
...
export function getStrengthLevel(lift: string, ratio: number): number {
  const standards = eliteStandards[lift as keyof typeof eliteStandards];
  if (!standards) return 0;

  if (ratio < standards.beginner) return ratio / standards.beginner * 0.25;
  if (ratio < standards.intermediate) return 0.25 + ((ratio - standards.beginner) / (standards.intermediate - standards.beginner)) * 0.25;
  if (ratio < standards.advanced) return 0.5 + ((ratio - standards.intermediate) / (standards.advanced - standards.intermediate)) * 0.25;
  if (ratio < standards.elite) return 0.75 + ((ratio - standards.advanced) / (standards.elite - standards.advanced)) * 0.25;
